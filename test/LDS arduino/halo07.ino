...
// PWM value.
#define BRIGHTNESS_FULL_OFF 0
#define BRIGHTNESS_FULL_ON 10
int bright2pwm[11] = {
  0,  1,  3,  7,
  15, 31, 63, 127,
  255, 511, 1023
...
#ifdef FULL_ON_OFF_THRESHOLD
// All led's are either on fully or off.  LEDs are ON if distance in mm is
// less that FULL_ON_OFF_THRESHOLD.
// If FULL_ON_OFF_THRESHOLD is not defined, then the modulated brightness
// function is compiled in.
byte brightness2(int d) {
  if (d <= FULL_ON_OFF_THRESHOLD) {
    return BRIGHTNESS_FULL_ON;
  }
  else {
    return BRIGHTNESS_FULL_OFF;
  }
}
