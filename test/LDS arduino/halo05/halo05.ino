...
#define LED0OFFSET 0
#define LED1OFFSET 265  // nominal: 270
#define LED2OFFSET 183  // nominal: 180
#define LED3OFFSET 92   // nominal: 90
...
void nextbrightness() {
  if (led0 == buffend) return; // Early out if puck spinning slow.
...
void getpacket() {
  ...
  while (!insync) {
    ...
    packetchar = Serial.read();
    while (packetchar < 0) packetchar = Serial.read();
